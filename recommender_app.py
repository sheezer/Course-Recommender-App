...
def train(model_name, params, selected_courses_df):
    ...
    if model_name == backend.models[0]:
        ...
        with st.spinner('Training...'):
            time.sleep(0.5)
            backend.train(model_name, params, selected_courses_df)
        st.success('Done!')
...
def predict(model_name, user_ids, params,selected_courses_df):
    ...
    with st.spinner('Generating course recommendations: '):
        time.sleep(0.5)
        res = backend.predict(model_name, user_ids, params, selected_courses_df)
...
# Clustering model
    ...
    cluster_no = st.sidebar.slider('Number of Clusters',
                                   min_value=0, max_value=100,
                                   value=20, step=1)
    enrollments = st.sidebar.slider('Students enrolled in popular courses',
    ...
    params['top_courses'] = top_courses
    params['enrollments'] = enrollments
# Clustering model with PCA
    ...
    top_courses = st.sidebar.slider('Top courses',
                                    min_value=0, max_value=100,
                                    value=10, step=1)
    components = st.sidebar.slider('Principal Components',
                                    min_value=0, max_value=100,
                                    value=10, step=1)
    params['cluster_no'] = cluster_no
    params['top_courses'] = top_courses
    ...
    # Add a slide bar for selecting top courses
    neighbours = st.sidebar.slider('Neighbours',
                                    min_value=0, max_value=100,
                                    value=10, step=1)
    top_courses = st.sidebar.slider('Top courses',
                                    min_value=0, max_value=100,
...
if training_button:
    train(model_selection, params, selected_courses_df)
...
if pred_button and selected_courses_df.shape[0] > 0:
    ...
    user_ids = [new_id]
    res_df = predict(model_selection, user_ids, params, selected_courses_df)
